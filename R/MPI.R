...
findPeaksPar2 <- function(arg) {
    ...
    list(scantime=xRaw@scantime, xRaw=xRaw, peaks=cbind(peaks, sample = rep.int(myID, nrow(peaks)))) ###Modified!!
}
...
fillPeaksChromPar <- function(arg) {
   ...
   require(coleXcms) ##Modified!!
    ...
    expand.mz <- params$expand.mz #added github
    expand.rt <- params$expand.rt #added github
	min.width.mz <- params$min.width.mz #added github
	min.width.rt <- params$min.width.rt #added github
	...
	peakrange[idx.expand.mz,"mzmin"]   =  rowMeans(   peakrange[ idx.expand.mz , c("mzmin","mzmax") , drop=FALSE ]  )   -min.width.mz/2 #added github  
	peakrange[idx.expand.mz,"mzmax"]   =  rowMeans(   peakrange[ idx.expand.mz , c("mzmin","mzmax") , drop=FALSE ]  )   +min.width.mz/2 #added github  
  
    
	idx.expand.rt = (peakrange[,"rtmax"] - peakrange[,"rtmin"]) < min.width.rt
	peakrange[idx.expand.rt,"rtmin"]   =  rowMeans(    peakrange[ idx.expand.rt , c("rtmin","rtmax") , drop=FALSE ]   )   -min.width.rt/2 #added github   
	peakrange[idx.expand.rt,"rtmax"]   =  rowMeans(    peakrange[ idx.expand.rt , c("rtmin","rtmax") , drop=FALSE ]   )   +min.width.rt/2 #added github  
  
  
	# Making sure the expanded peakrange doesn't exceed the RT range in the file
	peakrange[        peakrange[,"rtmin"]<min(lcraw@scantime)            ,"rtmin"]     =     min(lcraw@scantime) #added github
	peakrange[        peakrange[,"rtmax"]>max(lcraw@scantime)            ,"rtmax"]     =     max(lcraw@scantime) #added github
...
fillPeaksChromPar2 <- function(arg) {
  ...
  lcraw <- object@xcmsRaw[[myID]] #Modified. Need to make sure that order is preserved!!! 
  ...
  # Expanding the peakrange symmetrically by a factor of the total peak width 
  peakrange[,"mzmax"]  <-  peakrange[,"mzmax"]   +    (   (peakrange[,"mzmax"]-peakrange[,"mzmin"])/2    )*(expand.mz-1) #added github
  peakrange[,"mzmin"]  <-  peakrange[,"mzmin"]   -    (   (peakrange[,"mzmax"]-peakrange[,"mzmin"])/2    )*(expand.mz-1) #added github
  peakrange[,"rtmax"]  <-  peakrange[,"rtmax"]   +    (   (peakrange[,"rtmax"]-peakrange[,"rtmin"])/2    )*(expand.rt-1) #added github
  peakrange[,"rtmin"]  <-  peakrange[,"rtmin"]   -    (   (peakrange[,"rtmax"]-peakrange[,"rtmin"])/2    )*(expand.rt-1) #added github
